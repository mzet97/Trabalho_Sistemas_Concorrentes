...
void *produtor(void *arg)
{
    ...
    for (int i = 0; i < NUM_ITENS / NUM_PRODUTORES; i++)
    {
        ...
        printf("Produtor %d: aguardando item (INVERTIDO!)...\n", id);
        sem_wait(&item);
...
void *consumidor(void *arg)
{
    ...
    for (int i = 0; i < NUM_ITENS / NUM_CONSUMIDORES; i++)
    {
        ...
        printf("Consumidor %d: aguardando item...\n", id);
        sem_wait(&item);
...
int main()
{
    ...
    printf("=== PROTOCOLO INVERTIDO (DEADLOCK!) ===\n");
    printf("Iniciando sistema - observe o deadlock imediato\n\n");
