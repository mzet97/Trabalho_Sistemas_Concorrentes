...
void processo_filho(int numero_filho, MemoriaCompartilhada *mem, int semid) {
    ...
    if (numero_filho > 0) {
        if (setpriority(PRIO_PROCESS, 0, 19) == -1) {
            ...
            printf("Filho %d (PID: %d) reduziu prioridade para 19 (Starvation + Forced Sleep)\n", numero_filho, meu_pid);
        }
    } else {
        printf("Filho %d (PID: %d) manteve prioridade padrao (0)\n", numero_filho, meu_pid);
    }
...
int main(void) {
    ...
    printf("=== PROGRAMA DE SISTEMAS CONCORRENTES - ETAPA 4 (Starvation - Forçado) ===\n");
